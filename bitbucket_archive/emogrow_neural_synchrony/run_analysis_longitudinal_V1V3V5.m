function run_analysis_longitudinal_V1V3V5
%% Configuration
...
raw5 = EmoGrow.load_nirs('DBDOS','V5', nirs_data_dir);

raw1 = EmoGrow.fix_stims(raw1,true);
raw3 = EmoGrow.fix_stims(raw3,true);
raw5 = EmoGrow.fix_stims(raw5,true);
...
demo5 = nirs.createDemographicsTable(raw5);
goodsubs = intersect(demo1.Name, intersect(demo3.Name, demo5.Name) );

% Remove all but the good subjects
raw1=raw1(ismember(demo1.Name,goodsubs));
raw3=raw3(ismember(demo3.Name,goodsubs));
raw5=raw5(ismember(demo5.Name,goodsubs));
...
isV5 = strcmp(demo.Visit,'V5');

% Check that the subject order matches
assert(isequal(demo.Name(isV1),demo.Name(isV3)),'Subject mismatch');
assert(isequal(demo.Name(isV3),demo.Name(isV5)),'Subject mismatch');

sync_mean_puzzle_V1 = sync_mean_puzzle(isV1);
sync_mean_puzzle_V3 = sync_mean_puzzle(isV3);
sync_mean_puzzle_V5 = sync_mean_puzzle(isV5);
sync_mean_puzzle = [sync_mean_puzzle_V1, sync_mean_puzzle_V3, sync_mean_puzzle_V5];

sync_peak_puzzle_V1 = sync_peak_puzzle(isV1);
sync_peak_puzzle_V3 = sync_peak_puzzle(isV3);
sync_peak_puzzle_V5 = sync_peak_puzzle(isV5);
sync_peak_puzzle = [sync_peak_puzzle_V1, sync_peak_puzzle_V3, sync_peak_puzzle_V5];

sync_mean_freeplay_V1 = sync_mean_freeplay(isV1);
sync_mean_freeplay_V3 = sync_mean_freeplay(isV3);
sync_mean_freeplay_V5 = sync_mean_freeplay(isV5);
sync_mean_freeplay = [sync_mean_freeplay_V1, sync_mean_freeplay_V3, sync_mean_freeplay_V5];

sync_peak_freeplay_V1 = sync_peak_freeplay(isV1);
sync_peak_freeplay_V3 = sync_peak_freeplay(isV3);
sync_peak_freeplay_V5 = sync_peak_freeplay(isV5);
sync_peak_freeplay = [sync_peak_freeplay_V1, sync_peak_freeplay_V3, sync_peak_freeplay_V5];

%% Get change across visits (regression slope for each subject)
nsub = size(sync_mean_puzzle,1);
sync_mean_puzzle_slope = nan(nsub,1);
sync_peak_puzzle_slope = nan(nsub,1);
sync_mean_freeplay_slope = nan(nsub,1);
sync_peak_freeplay_slope = nan(nsub,1);

X = [1 2 3];
for i = 1:nsub
    
    if ~all(isnan(sync_mean_puzzle(i,:)))
        beta = glmfit(X,sync_mean_puzzle(i,:));
        sync_mean_puzzle_slope(i) = beta(end);
    end
    
    if ~all(isnan(sync_peak_puzzle(i,:)))
        beta = glmfit(X,sync_peak_puzzle(i,:));
        sync_peak_puzzle_slope(i) = beta(end);
    end
    
    if ~all(isnan(sync_mean_freeplay(i,:)))
        beta = glmfit(X,sync_mean_freeplay(i,:));
        sync_mean_freeplay_slope(i) = beta(end);
    end
    
    if ~all(isnan(sync_peak_freeplay(i,:)))
        beta = glmfit(X,sync_peak_freeplay(i,:));
        sync_peak_freeplay_slope(i) = beta(end);
    end
...
save('EmoGrow_DBDOS_4_extracted.mat',...
    'sync_mean_puzzle_V1','sync_mean_puzzle_V3','sync_mean_puzzle_V5','sync_mean_puzzle_slope',...
    'sync_peak_puzzle_V1','sync_peak_puzzle_V3','sync_peak_puzzle_V5','sync_peak_puzzle_slope',...
    'sync_mean_freeplay_V1','sync_mean_freeplay_V3','sync_mean_freeplay_V5','sync_mean_freeplay_slope',...
    'sync_peak_freeplay_V1','sync_peak_freeplay_V3','sync_peak_freeplay_V5','sync_peak_freeplay_slope' );

demo = demo(isV1,:);
demo.sync_mean_puzzle_V1 = sync_mean_puzzle_V1;
demo.sync_mean_puzzle_V3 = sync_mean_puzzle_V3;
demo.sync_mean_puzzle_V5 = sync_mean_puzzle_V5;
demo.sync_mean_puzzle_slope = sync_mean_puzzle_slope;

demo.sync_peak_puzzle_V1 = sync_peak_puzzle_V1;
demo.sync_peak_puzzle_V3 = sync_peak_puzzle_V3;
demo.sync_peak_puzzle_V5 = sync_peak_puzzle_V5;
demo.sync_peak_puzzle_slope = sync_peak_puzzle_slope;

demo.sync_mean_freeplay_V1 = sync_mean_freeplay_V1;
demo.sync_mean_freeplay_V3 = sync_mean_freeplay_V3;
demo.sync_mean_freeplay_V5 = sync_mean_freeplay_V5;
demo.sync_mean_freeplay_slope = sync_mean_freeplay_slope;

demo.sync_peak_freeplay_V1 = sync_peak_freeplay_V1;
demo.sync_peak_freeplay_V3 = sync_peak_freeplay_V3;
demo.sync_peak_freeplay_V5 = sync_peak_freeplay_V5;
demo.sync_peak_freeplay_slope = sync_peak_freeplay_slope;
