classdef AR_IRLS_OK < nirs.modules.AbstractGLM
...
%     specified explicitly in the stimulus design with BoxCar basis functions
    ...
    methods
        function obj = AR_IRLS_OK( prevJob )
        ...
        function S = runThis( obj, data )
            ...
            for i = 1:numel(data)
                ...
                if(rank([X C]) < size([X C],2) & obj.goforit)
                    ...
                    V=V(:,lst);
                    stats = nirs.math.ar_irls_ok( d, U(:,lst)*s(lst,lst), round(4*Fs) );
                    ...
                    % run regression
                    stats = nirs.math.ar_irls_ok( d, [X C], round(4*Fs) );
                end
                % put stats
                ncond = length(names);
