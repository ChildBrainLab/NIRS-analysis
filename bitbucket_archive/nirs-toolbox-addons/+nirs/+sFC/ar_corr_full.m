function [R,p,dfe]=ar_corr_full(data,modelorder,robust_flag)
if(nargin<3)
    robust_flag=true;
end
...
if(robust_flag)
    [R,p]=nirs.math.robust_corrcoef_full(yfilt,false,mask);
else
    [R,p]=nirs.math.corrcoef_full(yfilt,false,mask);
end
